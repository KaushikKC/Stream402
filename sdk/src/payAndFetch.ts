...
export async function payAndFetch(
  ...
  const conn =
    connection ||
    new Connection(
      challenge.network.includes("devnet")
  ...
  if (ownerAmount < amountRequired) {
    throw new Error(
      `Insufficient balance. Required: ${challenge.amount} ${
        challenge.currency
      }, Available: ${ownerAmount.toString()}`
    );
  }
  ...
  if (
    ...
    signature = await (walletAdapter as any).sendTransaction(tx, conn, {
      maxRetries: 5,
      skipPreflight: true,
    });
  } else if (
    "signTransaction" in walletAdapter &&
    typeof walletAdapter.signTransaction === "function"
  ) {
    // Fallback: sign and send manually
    const signedTx = await (walletAdapter as any).signTransaction(tx);
    ...
    throw new Error(
      "Wallet does not support transaction signing. Please use a wallet adapter with sendTransaction or signTransaction method."
    );
  }
  ...
  if (!receiptResponse.ok) {
    ...
    throw new Error(
      `Payment verification failed: ${error.error || receiptResponse.status}`
    );
  }

  const { accessToken } = (await receiptResponse.json()) as {
    accessToken: string;
  };
  ...
  if (!assetResponse.ok) {
    ...
    throw new Error(
      `Failed to get download URL: ${error.error || assetResponse.status}`
    );
  }

  const { url } = (await assetResponse.json()) as { url: string };
